 ...
 * No need to move image folders — just change the entries here.
 *
 * Structure:
 *   CATALOG is an array of organ system sections.
 *   Each section has:
 *     id        – unique slug, used in URLs (no spaces)
 *     label     – display name shown on the site
 *     icon      – Tabler icon name (ti-*)
 *     groups    – array of sub-groups within this section
 *
 *   Each group has:
 *     label     – display name
 *     diseases  – array of disease keys.
 *                 Each key MUST EXACTLY MATCH a folder name inside your images/
 *                 directory (case-sensitive on Linux/Mac).
 *
 * The "diseases" values are matched against image-manifest.json which is
 * generated by running:  python scan_images.py
 *
 * TIP: A disease can appear in multiple sections/groups — just list it twice.
 */
